    ...
    calculate (txt, index=0) {
        ...
        while(count < len(txt)) {
            ...
            if((char == '"' || char == "'") && (char == stringChar || stringChar == '')) {
                ...
                } else if(build[len(build)-1] == '@') {
                    // mem[a] (short read)
                    let value = build.split('@')[0];
                    while(build[len(build)-1] == '@') {
                        build = build.slice(0, len(build)-1);
                ...
                } else if(build[len(build)-1] == 'º') {
                    // angle in degrees
                    stack.push(Math.PI*(build.split('º')[0]/180));
